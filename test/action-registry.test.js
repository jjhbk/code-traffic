...
assert.throws(() => approvals.request({ capability: 'gmail.send', destination: 'alex@example.com', content: { subject: 'Missing thread', body: 'No thread.' } }, { principal: 'signal-box-user', surfaces: ['desktop'], expiresAt: Date.now() + 60_000 }), /destination, subject, body, and thread/);
assert.throws(() => approvals.request({ capability: 'calendar.update', eventId: 'event-1', changes: 'invalid' }, { principal: 'signal-box-user', surfaces: ['desktop'], expiresAt: Date.now() + 60_000 }), /event and changes/);
