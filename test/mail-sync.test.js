...
(async () => {
  ...
  const changedProvider = { async sync() { return { messages: [{ id: 'm1', threadId: 't1', from: 'client@example.com', to: 'me@example.com', subject: 'Request updated', body: 'The deadline moved to Monday.', historyId: 'h1' }], nextCursor: 'c3' }; } };
  const changedSync = new MailSync({ store, provider: changedProvider, clock: () => 1770000000000 });
  const changedResult = await changedSync.run({ adapterId: 'gmail:me', accountAddress: 'me@example.com' });
  assert.equal(changedResult.inserted, 1, 'changed provider observations are applied, not discarded');
  assert.equal(store.observations('gmail:me').find((item) => item.messageId === 'm1').subject, 'Request updated');
