...
(() => {
  ...
  const motion = context.processSensor({ sensor: 'motion', value: { active: true, activity: 'walking', confidence: 0.9 }, capturedAt: now, deviceId: 'phone-1' });
  assert.equal(motion.context.recordKey, 'mobile.sensor.motion');
  assert.equal(motion.context.value.activity, 'walking');
  assert.throws(() => context.processSensor({ sensor: 'motion', value: { active: 'yes', activity: 'walking' }, capturedAt: now }), /Motion context/);
