...
store.saveObservation({ observationId: 'o1', messageId: 'm1', threadId: 't1', subject: 'Friday handoff updated', body: "I'll send the revised handoff by Monday.", direction: 'outgoing', to: ['client@example.com'] }, 'fake');
const refreshed = service.processObservation(store.observations('fake').find((item) => item.observationId === 'o1'));
assert.equal(refreshed[0].reconciled, true, 'changed source evidence refreshes the existing task');
assert.equal(store.listTasks({ includeDismissed: true }).find((item) => item.taskId === taskId).summary, 'Friday handoff updated');
...
const orphanProactiveBeforeRemoval = store.exportData().data.jobs.filter((job) => job.kind === 'assistant.proactive-actions' && JSON.parse(job.payload_json).taskId === orphan.taskId).length;
...
assert.equal(store.exportData().data.jobs.filter((job) => job.kind === 'assistant.proactive-actions' && JSON.parse(job.payload_json).taskId === orphan.taskId).length, orphanProactiveBeforeRemoval + 1, 'source removal schedules immediate proactive reevaluation');
