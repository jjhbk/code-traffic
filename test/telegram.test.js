...
(async () => {
  ...
  const recoveryUpdate = { update_id: 901, message: { chat: { id: 42 }, text: '/assistant recover me' } };
  assert.equal(updateStore.claimTelegramUpdate(recoveryUpdate.update_id, recoveryUpdate), true);
  const recovered = [];
  const recoveryControl = new TelegramControl({ token: 'test-token', chatId: '42', listSessions: () => [], getHistory: () => ({ pendingQuestions: [] }), callbackStore: updateStore, assistantMessage: async (text) => recovered.push(text), getOffset: () => 902, fetchImpl: async (_url, options) => { recoveryControl.stopped = true; return { ok: true, json: async () => ({ ok: true, result: [] }) }; } });
  recoveryControl.stopped = false; await recoveryControl.poll();
  assert.deepStrictEqual(recovered, ['recover me']);
  assert.equal(updateStore.listRecoverableTelegramAssistantUpdates().length, 0);
  const recoveryCallback = { update_id: 902, callback_query: { id: 'recover-callback', data: 'unknown-callback', message: { chat: { id: 42 } } } };
  assert.equal(updateStore.claimTelegramUpdate(recoveryCallback.update_id, recoveryCallback), true);
  let callbackAttempts = 0;
  const callbackRecovery = new TelegramControl({ token: 'test-token', chatId: '42', listSessions: () => [], getHistory: () => ({ pendingQuestions: [] }), callbackStore: updateStore, getOffset: () => 903, fetchImpl: async () => {
    callbackAttempts += 1;
    if (callbackAttempts === 1) throw new Error('temporary Telegram outage');
    return { ok: true, json: async () => ({ ok: true, result: {} }) };
  } });
  callbackRecovery.stopped = false;
  await callbackRecovery.recoverAssistantUpdates();
  assert.equal(updateStore.listRecoverableTelegramUpdates().some((item) => item.updateId === 902), true, 'failed callbacks remain recoverable');
  await callbackRecovery.recoverAssistantUpdates();
  assert.equal(updateStore.listRecoverableTelegramUpdates().some((item) => item.updateId === 902), false, 'callback recovery completes after retry');
  callbackRecovery.stop();
  recoveryControl.stop();
  updateStore.close();
  callbackStore.close();
