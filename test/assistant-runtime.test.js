...
(async () => {
  ...
  assert.equal(runtime.health().jobs.overdue, 1);
  ...
  assert.equal(runtime.health().lastTickAt, now);
  assert.equal(runtime.health().lastTickJobs, 1);
  assert.equal(runtime.health().lastError, null);
  assert.equal(runtime.health().jobs.overdue, 0);
