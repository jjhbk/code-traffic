...
const corrected = assistant.handle({ conversationId: 'desktop:test', text: `correct ${candidate.taskId} summary: Reply to Alex about the revised proposal`, externalId: 'desktop-message-correction' });
assert.match(corrected.response, /Updated/);
assert.equal(store.listTasks({ includeDismissed: true }).find((task) => task.taskId === candidate.taskId).summary, 'Reply to Alex about the revised proposal');
...
const goal = assistant.handle({ conversationId: 'desktop:test', text: 'goal launch: Ship the first proactive assistant pilot', externalId: 'desktop-message-goal' });
assert.match(goal.response, /track the goal/);
const goals = assistant.handle({ conversationId: 'desktop:test', text: 'my goals', externalId: 'desktop-message-goals' });
assert.match(goals.response, /launch: Ship the first proactive assistant pilot/);
assert.equal(store.listContext({ recordType: 'goal' }).length, 1);
const forgotten = assistant.handle({ conversationId: 'desktop:test', text: 'forget preference response style', externalId: 'desktop-message-forget' });
assert.match(forgotten.response, /Forgot preference/);
assert.equal(store.getContext('preference', 'response style'), null);
