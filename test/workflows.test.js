...
assert.equal(workflow.cancel(started.workflowId).state, 'cancelled');
...
const completed = workflow.start({ workflowType: 'completed-fixture', payload: {} });
workflow.transition(completed.workflowId, 'completed');
assert.throws(() => workflow.transition(completed.workflowId, 'ready'), /Completed workflows/);
...
assert.ok(store.exportData().data.workflows.length === 3);
store.close();
