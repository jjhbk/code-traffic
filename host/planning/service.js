class PlanningService {
  ...
  async executeDecision({ decision, recipe, inputs = {}, grantId, taskId = null, principal = 'signal-box-user', surface = 'desktop', executor = null } = {}) {
    ...
    const workflow = this.workflows.start({ workflowType: 'browser-action', taskId, state: 'evaluating', payload: { recipeId: registeredRecipe.id, inputs, grantId, principal, surface, decisionReason: decision.reason || null } });
    ...
    try {
      const result = await this.browserActions.executeWithStandingGrant(registeredRecipe, inputs, { grantId, principal, surface, taskId, executor: executor || undefined });
