...
4. **Connector pagination — partially implemented.** Gmail history/bootstrap and Calendar page continuations now use versioned opaque cursors and have restart fixtures. Background sync now applies provider results, cursors, observations and reconciliation in the durable worker through a credential-free remote-provider adapter, and a durable expiring connector lease prevents manual and background sync overlap across processes. Drive page tokens remain provider cursors and need explicit bootstrap/change checkpoint semantics, deletion handling, and interruption tests.
