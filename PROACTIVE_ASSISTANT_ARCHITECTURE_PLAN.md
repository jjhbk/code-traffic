...
9. **Telegram durability — improved, still partial.** Assistant messages, digest feedback, polling offsets, and generated callback mappings now persist in SQLite with chat scoping, expiry, claim/consume semantics, and restart coverage. Approval/action execution still depends on the Electron-owned provider and terminal bridges, and queued-message recovery needs completion.
