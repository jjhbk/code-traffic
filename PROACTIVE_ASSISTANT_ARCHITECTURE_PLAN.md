...
2. **Approval consumption — implemented for supported writes.** Browser, Gmail, and Calendar dispatch claim one execution attempt transactionally before provider work. Unknown outcomes remain reconcilable; broader action-registry coverage is still needed as new providers are added.
...
8. **Scheduling/model work — partially implemented.** Model ranking is skipped outside an eligible digest slot and JSON model clients have request timeouts. Dirty-source scheduling, concurrency budgets, and frontier worker cancellation still need hardening.
9. **Telegram durability — partially implemented.** Durable assistant messages and digest feedback exist, but general callback mappings and polling offsets are still in memory and queued-message recovery needs completion.
