...
4. **Connector pagination — partially implemented.** Gmail history/bootstrap, Calendar page continuations and Drive bootstrap/change page continuations now use versioned opaque cursors and restart fixtures. Drive bootstrap captures its change checkpoint before file enumeration, and sync application remains transactional so deletion and interrupted-run behavior do not advance a checkpoint prematurely. Background sync now applies provider results, cursors, observations and reconciliation in the durable worker through a credential-free remote-provider adapter, and a durable renewing connector lease prevents manual and background sync overlap across processes while fencing lost owners. Live Google-account pagination, token expiry and large-account behavior remain unverified.
