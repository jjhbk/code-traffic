...
| `host/runtime/background-host-worker.js` | Durable scheduling, connector cursors, observation/task reconciliation and proactive eligibility | Provider credentials and network calls remain parent-owned; replace the parent IPC dependency with a packaged credential-capable host boundary |
...
1. **Calendar authority — implemented locally.** `calendar.update` is registered by policy, approval is required, provider edits use etags, and desktop/mobile dispatch consume the approval. A local HTTP sandbox now verifies the GET/PUT and `If-Match` authority boundary; live Google Calendar conflict and guest-notification behavior remain unverified.
2. **Approval consumption — implemented for supported writes.** Browser, Gmail, and Calendar dispatch claim one execution attempt transactionally before provider work. Gmail and Calendar now share `host/actions/execution-service.js`, which owns preflight validation, claim/transition/receipt sequencing and unknown-outcome retention; browser approvals and automatic decisions resolve registered recipes by ID and digest rather than relying on an Uber-only dispatch path. Explicit `gmail.send` and `calendar.update` task automations can now use durable standing-grant runs through `host/actions/provider-autonomous-service.js`, including provider verification for unknown outcomes. Standing-grant consumption and autonomous-run creation are atomic; provider and browser recovery jobs fence authorized/dispatched runs after restart instead of replaying uncertain effects. The action registry now validates typed Gmail and Calendar payloads before dispatch; broader non-browser provider coverage remains future expansion.
3. **Digest feedback — implemented.** Telegram feedback uses deterministic notification-bound callback IDs and writes through the durable notification feedback store. Generated callback mappings, polling offsets and callback/update claim state persist across restart; callback queries now remain recoverable after a transport failure.
4. **Connector pagination — partially implemented.** Gmail history/bootstrap, Calendar page continuations and Drive bootstrap/change page continuations now use versioned opaque cursors and restart fixtures. Drive bootstrap captures its change checkpoint before file enumeration, sync application remains transactional so deletion and interrupted-run behavior do not advance a checkpoint prematurely, and same-identity observation updates are applied rather than discarded. Background sync now applies provider results, cursors, observations and reconciliation in the durable worker through a credential-free remote-provider adapter, and a durable renewing connector lease prevents manual and background sync overlap across processes while fencing lost owners. Live Google-account pagination, token expiry and large-account behavior remain unverified.
...
6. **Send verification — hardened.** Gmail sends carry an attempt marker; reconciliation can require that marker and also checks send time and account identity before falling back to content matching. Providers now accept injectable API roots for local HTTP sandboxing, and the Gmail send/reconcile path has an end-to-end local HTTP fixture. Live provider behavior and account-level delivery remain unverified.
7. **Browser execution — bounded verification implemented.** The Uber booking recipe now requires a confirmation postcondition, missing confirmation fails rather than becoming success, and registered recipes carry a normalized HTTP(S) origin allowlist enforced for navigation. Standing-grant browser decisions delegated from the background host carry the evaluated task version through planning and perform a final pre-dispatch freshness check; unavailable browser sessions now produce deduplicated attention notifications instead of silent skips. General browser intelligence and live-provider markup/booking validation remain out of scope for fixtures.
8. **Scheduling/model work — substantially implemented locally.** Model ranking is skipped outside an eligible digest slot and JSON model clients have request timeouts. The background worker now owns deterministic proactive eligibility and attention-notification enqueueing, binding delegated execution decisions to strictly monotonic task versions; browser and provider actions now share one per-cycle concurrency frontier. Mobile context changes, browser reconnects, source changes and dependency transitions enqueue durable reevaluation jobs with bounded deduplication. Live model quality, long-running frontier cancellation and production workload tuning remain unverified.
9. **Telegram durability — improved, still partial.** Assistant messages, digest feedback, polling offsets, generated callback mappings, and processed update IDs now persist in SQLite with chat scoping, expiry, claim/consume semantics, callback retry and restart coverage. Claimed `/assistant` and callback updates recover after a crash while legacy terminal/session commands remain non-replayable; approval/action execution still depends on the Electron-owned provider and terminal bridges.
10. **Privacy boundary — partially implemented.** Nested payloads are recursively sanitized and frontier paths fail closed when local recognition is unavailable. Gmail deletion now removes related observations, tasks, provider approvals, autonomous payloads, standing grants, notifications, proactive jobs, and task-linked conversation messages transactionally. Local regex fallback remains intentionally conservative, not an anonymity guarantee; broader private-operation schemas and deletion audits remain required.
11. **Release gates — verified locally (2026-09-17).** Electron packaged-resource, packaging metadata, browser-extension, Linux x64 packaging, `.deb` and `.rpm` distributable generation, and unsigned-build signing checks pass. The separate Expo mobile project exports an Android bundle successfully. These are build validations only; real-device installation, TLS/pinning, live Google accounts, browser markup drift, and provider sandbox behavior remain unverified.
...
Run a packaging spike before selecting the launcher. `node:sqlite` requires Node 22.5.0 or newer in development/test runtimes (the packaged Electron runtime is also Node 22), credentials depend on Electron safeStorage, Forge disables RunAsNode, and browser bridge/PTY ownership currently sit in the app. Do not assume a plain Node child or a UI-owned child will provide independent operation. Choose a supported packaged host process and credential interface after platform validation; an Electron-based background entry is a candidate, not a settled implementation.
