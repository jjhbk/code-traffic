...
11. **Release gates — verified locally (2026-09-17).** Electron packaged-resource, packaging metadata, browser-extension, Linux x64 packaging, and unsigned-build signing checks pass. The separate Expo mobile project exports an Android bundle successfully. These are build validations only; real-device installation, TLS/pinning, live Google accounts, browser markup drift, and provider sandbox behavior remain unverified.
